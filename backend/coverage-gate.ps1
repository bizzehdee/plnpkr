#!/usr/bin/env pwsh
# Enforces the coverage gate: >= 90% line AND branch on TeamTools.Core.
...
dotnet test (Join-Path $here 'tests/TeamTools.Core.Tests') `
...
$pkg = $xml.coverage.packages.package | Where-Object { $_.name -eq 'TeamTools.Core' }
if (-not $pkg) { Write-Error 'TeamTools.Core not found in coverage report.'; exit 1 }
...
Write-Host ''
Write-Host "TeamTools.Core coverage — line: $('{0:P1}' -f $line), branch: $('{0:P1}' -f $branch) (gate: $thresholdPct)"
